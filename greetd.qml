...
ShellRoot {
    ...
    Process {
        id: getDEsProcess
        running: true
        command: ["bash", "-c", "find /usr/share/wayland-sessions/ -name '*.desktop' 2>/dev/null | while read f; do name=$(grep '^Name=' \"$f\" | cut -d= -f2-); exec=$(grep '^Exec=' \"$f\" | cut -d= -f2-); case \"$exec\" in uwsm*) continue;; esac; echo \"$name|||$exec\"; done"]
        stdout: StdioCollector {
            onStreamFinished: {
