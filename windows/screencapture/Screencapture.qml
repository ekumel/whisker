    ...
    LazyLoader {
        ...
        component: PanelWindow {
            ...
            Component.onCompleted: {
                ...
                root.tempScreenshot = "/tmp/whisker_screenshot_" + ts + ".png";
            }
            ...
            Component {
                ...
                Process {
                    ...
                    onExited: (code) => {
                        Log.info("screencap", "ffmpeg exited: " + code);
                        success = code === 0;

                        if (success) {
                            Log.info("screencap", "copying to clipboard");
                            Quickshell.execDetached({
                        ...
                        win.processing = false;
                        Log.info("screencap", "done, closing");
                        win.close();
                        destroy();
                    }
                }
            ...
            function saveRegion(rect, suffix) {
                Log.info("screencap", "saveRegion started: " + rect.x + "," + rect.y + " " + rect.width + "x" + rect.height);
                screencopy.grabToImage(function(result) {
                    Log.info("screencap", "full screenshot grabbed for cropping");
                    if (!result.saveToFile(root.tempScreenshot)) {
                        Log.info("screencap", "ERROR: failed to save temp screenshot");
                        win.savedSuccess = false;
                        win.processing = false;
                        win.close();
                        return;
                    ...
                    Log.info("screencap", "temp saved, cropping with ffmpeg");
                    var ts = Qt.formatDateTime(new Date(), "yyyy-MM-dd_hh-mm-ss");
            ...
            ScreencopyView {
                ...
                onHasContentChanged: {
                    ...
                    if (hasContent) {
                        Log.info("screencap", "grabbing for preview");
                        grabToImage(function(result) {
                            Log.info("screencap", "preview grabbed: " + result.url);
                ...
                Item {
                    ...
                    Repeater {
                        ...
                        delegate: Item {
                            ...
                            property real barX: Preferences.verticalBar() ? Appearance.barSize : 0
                            property real barY: Preferences.horizontalBar() ? Appearance.barSize : 0
